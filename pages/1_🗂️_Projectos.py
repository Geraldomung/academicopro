...
# No início do seu app, adicione verificação de dependências
import subprocess
import sys
...
for package in ['spacy', 'language_tool_python', 'nltk']:
    check_package(package)

# Após instalar, carregue o modelo português
try:
    import spacy
...
def initialize_session_state():
    ...
    defaults = {
        ...
        'referencial_teorico_sugestoes_gemini': [],
        'autores_citados': [],
...
def get_project_data():
    ...
    return {
        ...
        'referencial_teorico_sugestoes_gemini': st.session_state.referencial_teorico_sugestoes_gemini,
...
def avaliar_qualidade_documento(projeto):
    secoes = {
        ...
        "Referencial Teórico": '\n'.join(projeto.get('referencial_teorico_sugestoes_gemini', [])),
...
def regenerar_secao(secao_nome, projeto):
    if secao_nome == "Introdução":
        ...
        full_text = "\n".join([
            projeto.get('introducao', ''), projeto.get('problematizacao', ''), projeto.get('referencial_teorico_sugestoes_gemini', ''),
...
def generate_tema_suggestions_gemini(area_interesse, curso, palavras_chave, nivel_academico):
    ...
    Use a construção frasica correcta respeitando paragrafos, periodos, orações e pontuacao correcta.
...
def generate_inquiry_gemini(problema_pesquisa, objetivo_geral, objetivos_especificos, tipo_inquerito):
    ...
    if tipo_inquerito == "Perguntas de Pesquisa":
        ...
        e nos Objetivos Específicos:
        {objetivos_str}
        Primeiro defina hipoteses de pesquisa, que são suposições testáveis sobre a relação entre variáveis., cite alguns autores oara sustentar a sua definição.    
        ...
        Use a construção frasica correcta respeitando paragrafos, periodos, orações e pontuacao correcta.
        Nunca use conteudos detectados de IA.
        Não retorne comentários desnecessarios para a pesquisa, apenas as hipoteses.
        Enumere as hipoteses de forma clara e direta, sem rodeios.
       Tenha cuidado com o tempo verbal correcto, use sempre o futuro do presente ou o futuro do pretérito. 
...
def generate_objetivos_suggestions_gemini(tema, problema, curso, local_foco):
    ...
    O objetivo geral deve ser uma frase completa que sintetize a intenção principal da pesquisa, enquanto os objetivos específicos devem ser ações concretas que ajudem a alcançar o objetivo geral.
    Formate a resposta EXATAMENTE assim:

    OBJETIVO GERAL: [Sugerir um Objectivo Geral claro e direto, que sintetize a intenção principal da pesquisa]
...
# def generate_referencial_teorico_suggestions_geminis(tema, problema, curso, local_foco, objetivo_geral, objetivos_especificos):
...
def create_word_document(data):
 ...
 # 3. Referencial Teórico
        ...
        if data.get('referencial_teorico_sugestoes_gemini'):
            for i, topico in enumerate(data['referencial_teorico_sugestoes_gemini']):
...
def main():
    ...
    if st.session_state.passo == 1:
        ...
        with col2:
            ...
            nivel_academico = st.selectbox(
                "Nível Acadêmico:",
                ["Licenciatura", "Mestrado", "Doutorado", "Pós-doc", "Outro"],
                key="nivel_academico_select",
                index=["Licenciatura", "Mestrado", "Doutorado", "Pós-doc", "Outro"].index(
    ...
    # st.subheader("🔍 Buscar Referências Automaticamente")
        ...
        with col1:
            if st.button("✨ Gerar Referencial Completo", key="gerar_referencial_completo"):
                with st.spinner("Buscando referências e gerando sugestões..."):
                    st.session_state.referencial_teorico_sugestoes_gemini = generate_referencial_teorico_suggestions_gemini(
                        tema=st.session_state.tema,
        ...
        with col2:
            ...
            #     st.session_state.referencial_teorico_sugestoes_gemini = buscar_referencias_serpapi(st.session_state.tema)
        
        # Edição do referencial
            if st.session_state.referencial_teorico_sugestoes_gemini:
                edited_ref = st.text_area(
                    "Edite seu referencial teórico:",
                    value="\n\n".join(st.session_state.referencial_teorico_sugestoes_gemini),
                    height=400
                )
                st.session_state.referencial_teorico_sugestoes_gemini = [r.strip() for r in edited_ref.split("\n\n") if r.strip()]
                        ...
                        for i, ref in enumerate(referencias):
                            with st.expander(f"{ref['titulo']} ({ref['ano']})"):
                                ...
                                if st.button(f"Adicionar ao Referencial", key=f"add_ref_{i}"):
                                    ...
                                    st.session_state.referencial_teorico_sugestoes_gemini.append(citacao)
                                    st.rerun()
                    else:
        ...
        if st.session_state.referencial_teorico_sugestoes_gemini:
            ...
            st.session_state.referencial_teorico_sugestoes_gemini = [line.strip() for line in edited_referencial_str.split('\n') if line.strip()]
            ...
            with col2_nav:
                if st.button("Próximo ➡️", key="passo7_next"):
                    st.session_state.autores_citados = extract_authors_from_referencial(st.session_state.referencial_teorico_sugestoes_gemini)
        ...
        REFERENCIAL TEÓRICO: {'; '.join(st.session_state.referencial_teorico_sugestoes_gemini)}
...
# 
    ...
    if 'secao_a_regenerar' in st.session_state:
        ...
        if secao == "Introdução":
            ...
            st.session_state['referencial_teorico_sugestoes_gemini'] = nova_versao
