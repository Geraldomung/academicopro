...
## 4. Funções de Análise e Sugestões
...
with st.sidebar:
    ...
    indice_selecionado = st.session_state.secao_atual - 1
    ...
    # Atualiza a seção atual se o usuário selecionar uma diferente
    for passo_num, label in passos_labels.items():
        if label == secao_selecionada_label and st.session_state.secao_atual != passo_num:
...
if st.session_state.secao_atual == 1:
    ...
    if palavras < 10:
        if st.button("✨ Sugerir título completo"):
            sugestao = gerar_expansao_gemini(
                "Título",
                st.session_state.artigo['titulo'],
                {'palavras_faltantes': 10-palavras, 'paragrafos_faltantes': 1}
            )
    ...
    with col1_nav:
        ''
    with col2_nav:
        if st.button("Próximo ➡️", key="passo1_next"):
    ...
    if palavras < 100 or paragrafos < 1:
        if st.button(f"✨ Gerar resumo completo (faltam {max(0, 100-palavras)} palavras)"):
            sugestao = gerar_expansao_gemini(
                "Resumo",
                st.session_state.artigo['resumo'],
                {'palavras_faltantes': max(100-palavras, 50), 'paragrafos_faltantes': 1}
            )
            ...
            with st.spinner("Analisando resumo e gerando sugestões..."):
                try:
                    ...
                    st.session_state.sugestao_melhoria_resumo = {
                        'texto': sugestao,
                        'resumo_original': st.session_state.artigo['resumo']
                    }
                except Exception as e:
    ...
    if palavras < 200 or paragrafos < 3:
        if st.button(f"✨ Gerar introdução completa (faltam {max(0, 200-palavras)} palavras)"):
            sugestao = gerar_expansao_gemini(
                "Introdução",
                st.session_state.artigo['introducao'],
                {'palavras_faltantes': max(200-palavras, 150), 'paragrafos_faltantes': max(3-paragrafos, 2)}
            )
    ...
    st.markdown("""**Instruções:**  
    - Descreva detalhadamente os métodos utilizados  
    - Inclua tipo de estudo, população, instrumentos e procedimentos  
    - Seja claro e replicável  
    - Tamanho sugerido: 2-4 páginas  
    """)
    exibir_warnings_secao('metodologia')
    
    if st.button("✨ Gerar Metodologia com Gemini"):
        context = get_global_context()
    
        if not context['resumo'] or not context['introducao']:
            st.warning("""**Pré-requisitos incompletos!**  
                ...
                try:
                    ...
                    if conteudo_gerado:
                        ...
                        if problemas_coerencia:
                            st.warning("""**Ajustes necessários detectados:**  
    ...
    if palavras < 300 or paragrafos < 2:
        if st.button(f"✨ Gerar metodologia completa (faltam {max(0, 300-palavras)} palavras)"):
            sugestao = gerar_expansao_gemini(
                "Metodologia",
                st.session_state.artigo['metodologia'],
                {'palavras_faltantes': max(300-palavras, 200), 'paragrafos_faltantes': max(2-paragrafos, 1)}
            )
    ...
    if palavras < 400 or paragrafos < 3:
        if st.button(f"✨ Gerar resultados completos (faltam {max(0, 400-palavras)} palavras)"):
            sugestao = gerar_expansao_gemini(
                "Resultados",
                st.session_state.artigo['resultados'],
                {'palavras_faltantes': max(400-palavras, 300), 'paragrafos_faltantes': max(3-paragrafos, 2)}
            )
    ...
    """)
    exibir_warnings_secao('conclusao')
    ...
    if palavras < 200 or paragrafos < 2:
        if st.button(f"✨ Gerar conclusão completa (faltam {max(0, 200-palavras)} palavras)"):
            sugestao = gerar_expansao_gemini(
                "Conclusão",
                st.session_state.artigo['conclusao'],
                {'palavras_faltantes': max(200-palavras, 150), 'paragrafos_faltantes': max(2-paragrafos, 1)}
            )
    ...
    if len(refs) < 5:
        if st.button("✨ Sugerir referências adicionais"):
            sugestao = gerar_expansao_gemini(
                "Referências",
                st.session_state.artigo['referencias'],
                {'palavras_faltantes': 0, 'paragrafos_faltantes': max(5-len(refs), 3)}
            )
    ...
    with col1_nav:
        if st.button("⬅️ Anterior", key="passo8_prev"):
            ...
            st.rerun()
    with col2_nav:
        if st.button("🔄 Começar Novo Artigo", key="novo_artigo"):
...
with col1:
    if st.button("🔍 Analisar Artigo"):
        with st.spinner("Analisando qualidade do artigo..."):
